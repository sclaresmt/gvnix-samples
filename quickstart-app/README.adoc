...
At this point, you can run your generated gvNIX application. You have different ways to execute it:

. Using console:
+

You can use the following commands to execute your application on tomcat server:

[source,shell]
mvn clean compile tomcat:run

image:images/run-application.png[Running gvNIX application]

. You can use STS to run your application:
